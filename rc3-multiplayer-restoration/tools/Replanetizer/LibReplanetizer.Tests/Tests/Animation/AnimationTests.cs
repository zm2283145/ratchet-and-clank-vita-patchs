...
namespace LibReplanetizer.Tests.Animation
{
    public class BoneDataTests
    {
        ...
        [Fact]
        public void Serialize_Vita_WritesParentBeforeUnknownFlag()
        {
            byte[] original = BuildBoneDataBlock(0, 1024f, 2048f, 512f, 0x7000, 0x80);
            var bone = new BoneData(GameType.RaC3, original, 0);
            byte[] serialized = bone.Serialize(vita: true);

            Assert.Equal(0x80, ReadShort(serialized, 0x0C));
            Assert.Equal(0x7000, ReadShort(serialized, 0x0E));
        }
    ...
    public class BoneMatrixTests
    {
        ...
        public void Serialize_Vita_WritesParentBeforeUnknownFlag()
        {
            var transform = new Matrix3x4(
                ...
                0f, 1f, 0f, 0f,
                0f, 0f, 1f, 0f);
            byte[] original = BuildBoneMatrixBlock(
                0, transform, Vector3.Zero, 0x7000, (short) (2 * 0x40));
            var matrix = new BoneMatrix(GameType.RaC3, original, 0);
            byte[] serialized = matrix.Serialize(vita: true);

            Assert.Equal(0x80, ReadShort(serialized, 0x3C));
            Assert.Equal(0x7000, ReadShort(serialized, 0x3E));
        }
    }
    ...
    public class ModelSoundTests
    {
        ...
        [Fact]
        public void Serialize_Vita_PreservesOff18ByteOrder()
        {
            byte[] original = BuildSoundBlock(0, 1, 10.0f, 100, 50, 5, 3, 0x1234, 42, 0xABCD);
            var sound = new ModelSound(original, 0);
            byte[] serialized = sound.Serialize(vita: true);

            Assert.Equal(0x12, serialized[0x18]);
            Assert.Equal(0x34, serialized[0x19]);
        }
