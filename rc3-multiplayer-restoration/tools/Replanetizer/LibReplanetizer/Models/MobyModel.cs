...
namespace LibReplanetizer.Models
{
    public class MobyModel : MetalModel
    {
        ...
        private byte[] sourceIndexAttachmentBlock = [];
        ...
        private int sourceMeshPointer = 0;
        private int sourceFirstPayloadPointer = 0;
        ...
        private MobyModel(FileStream fs, GameType game, short modelID, int offset, bool standaloneModel)
        {
            ...
            int[] sourcePayloadPointers =
            [
                collisionPointer, soundPointer, attachmentPointer,
                boneMatrixPointer, boneDataPointer,
                banglesPointer * 0x10, corncobPointer * 0x10
            ];
            foreach (int pointer in sourcePayloadPointers)
            {
                if (pointer > 0 && (sourceFirstPayloadPointer == 0 || pointer < sourceFirstPayloadPointer))
                    sourceFirstPayloadPointer = pointer;
            }
            ...
            if (banglesPointer > 0)
            {
                ...
                for (int i = 0; i < banglesCount; i++)
                {
                    ...
                    {
                        // Unknown bangle records are packed by populated entry,
                        // even when the occupancy mask contains holes.
                        bangles.Add(new Bangle(fs, offset, bangleHeaderOffset,
                            banglesPointer * 0x10 + 0x40 + bangleDataIndex * 0x10));
                        bangleDataIndex++;
                    }
                    else
                        bangles.Add(null);
            ...
            if (attachmentPointer > 0)
            {
                ...
                {
                    // These records occupy a fixed 0x10-byte slot in the PS3 and
                    // Vita engines.  The terminating 0xff can fall in the block
                    // that follows, so retain the actual slot independently of
                    // the parser's logical list.
                    sourceIndexAttachmentBlock = ReadBlock(fs, offset + attachmentPointer, (id == 1 || id == 2) ? 0x20 : 0x10);
                    int attid = 0;
                    while (true)
                    {
        ...
        public int WriteBytes(FileStream fs, FileStream? vitaVertexFile = null)
        {
            ...
            if (id == 0)
                SeekReserve(fs, (vitaVertexFile != null) ? 0x110 : 0x20 * 4, 0x01);
            else if (id != 1 && id != 2)
                SeekReserve(fs, 0x20, 0x01);

            // The space between the animation pointer list and the mesh header is
            // model-specific.  The PS3 and Vita builds retain the same relative
            // mesh-header offset, so preserve it when producing a split Vita model.
            if (vitaVertexFile != null && hasMeshData && sourceMeshPointer > 0)
            {
                int currentRelativeOffset = (int) fs.Position - headerOffset;
                if (currentRelativeOffset < sourceMeshPointer)
                    SeekReserve(fs, sourceMeshPointer - currentRelativeOffset, 0x01);
            }
            ...
            byte[] vertexBytes = (vitaVertexFile != null) ? SerializeVitaMobyVertices() : SerializeVertices();
            byte[] faceBytes = GetFaceBytes();

            FileStream payloadFile = vitaVertexFile ?? fs;
            int vertOffset = SeekWriteForced(payloadFile, vertexBytes, (vertexBytes.Length > 0) ? 0x80 : 0x10);
            int metalVertOffset = SeekWrite(payloadFile,
                (vitaVertexFile != null) ? SerializeVitaMetalVertices() : SerializeMetalVertices(), 0x01);
            int faceOffset = SeekWriteForced(payloadFile, faceBytes);
            int metalIndexOffset = SeekWrite(payloadFile, SerializeMetalIndices(), 0x01);

            // Meshless models keep their original structural layout in the Vita
            // engine. Preserve the gap before their first payload section.
            if (vitaVertexFile != null && !hasMeshData && sourceFirstPayloadPointer > 0)
            {
                int currentRelativeOffset = (int) fs.Position - headerOffset;
                if (currentRelativeOffset < sourceFirstPayloadPointer)
                    SeekReserve(fs, sourceFirstPayloadPointer - currentRelativeOffset, 0x01);
            }
            ...
            if (bangles.Count > 0)
            {
                ...
                for (int i = 0; i < bangles.Count; i++)
                {
                    ...
                    if (bangle != null)
                    {
                        bangleOffset = bangle.WriteBytes(fs, headerOffset, unkBanglesDataPointer + bangleIndex * 0x10, vitaVertexFile);
                        bangleIndex++;
                    }
                    else
                    {
            ...
            int collisionDataOffset = (collisionData != null)
                ? SeekWrite(fs, collisionData.Serialize(vitaVertexFile != null))
                : 0;
            ...
            if (modelSounds.Count > 0)
            {
                ...
                for (int i = 0; i < modelSounds.Count; i++)
                    SeekWrite(fs, modelSounds[i].Serialize(vitaVertexFile != null), 0x01);
            }
            ...
            else if (indexAttachments.Count > 0)
            {
                ...
                if (vitaVertexFile != null && sourceIndexAttachmentBlock.Length > 0)
                {
                    attachmentBytes = (byte[]) sourceIndexAttachmentBlock.Clone();
                }
                else
                {
                    int attachmentSize = 0x04 + indexAttachments.Count + 0x01;
                    attachmentBytes = new byte[attachmentSize];
            ...
            if (id != 1 && id != 2)
            {
                if (boneMatrices.Count > 0)
                {
                    ...
                    for (int i = 0; i < boneMatrices.Count; i++)
                        SeekWrite(fs, boneMatrices[i].Serialize(vitaVertexFile != null), 0x01);
                }
                else
                {
                    boneMatrixOffset = (vitaVertexFile != null) ? (int) fs.Position : SeekPast(fs, 0x10);
                }
            }
            ...
            if (id != 1 && id != 2)
            {
                if (boneDatas.Count > 0)
                {
                    ...
                    for (int i = 0; i < boneDatas.Count; i++)
                        SeekWrite(fs, boneDatas[i].Serialize(vitaVertexFile != null), 0x01);
                }
                else
                {
                    boneDataOffset = (vitaVertexFile != null) ? (int) fs.Position : SeekPast(fs, 0x10);
                }
            }
            else
            {
            ...
            {
                int animOffset = animations[i].WriteBytes(fs, headerOffset, vitaVertexFile != null);
            ...
            if (corncobs.Length == 16)
            {
                ...
                for (int i = 0; i < 16; i++)
                {
                    ...
                    if (corncob != null)
                    {
                        byte[] corncobBytes = corncob.Serialize(vitaVertexFile != null);
            ...
            {
                // Mixed-endian field: Vita swaps the first PS3 byte pair while
                // preserving the second pair.
                headerBytes[0x44] = (byte) (unk6 >> 16);
                headerBytes[0x45] = (byte) (unk6 >> 24);
                headerBytes[0x46] = (byte) (unk6 >> 8);
                headerBytes[0x47] = (byte) unk6;
            }
            else
            {
            ...
            if (meshDataOffset > 0)
            {
                ...
                WriteInt(meshDataBytes, 0x10, (vitaVertexFile != null) ? vertOffset : GetRelativeOffset(vertOffset, headerOffset));
                WriteInt(meshDataBytes, 0x14, (vitaVertexFile != null) ? faceOffset : GetRelativeOffset(faceOffset, headerOffset));
            ...
            for (int i = 0; i < textureConfig.Count; i++)
            {
                ...
                WriteInt(textureConfigBytes, i * 0x10 + 0x0C,
                    (vitaVertexFile != null)
            ...
            for (int i = 0; i < metalTextureConfig.Count; i++)
            {
                ...
                WriteInt(metalTextureConfigBytes, i * 0x10 + 0x0C,
                    (vitaVertexFile != null)
