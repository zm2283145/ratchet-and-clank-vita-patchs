...
namespace LibReplanetizer.Models
{
    ...
    public class MobyModelCollisionPrimitive
    {
        ...
        public byte[] Serialize(bool vita = false)
        {
            ...
            int serializedValue0 = value0;
            if (vita && shape == MobyModelCollisionShape.IndexedCapsule)
            {
                // Indexed capsules store two vertex indices as independent
                // 16-bit values. Vita keeps their order and endian-swaps each.
                serializedValue0 = (int) System.Numerics.BitOperations.RotateRight((uint) value0, 16);
            }
            WriteInt(outbytes, 0x04, serializedValue0);
    ...
    public class MobyModelCollision
    {
        ...
        public byte[] Serialize(bool vita = false)
        {
            ...
            {
                primitive.Serialize(vita).CopyTo(outbytes, offset);
